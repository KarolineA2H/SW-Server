...
public class Communication implements ARQProtocol.DisconnectedListener {
    ...
    public void sendOrderToRobotWithRot(int address, int x, int y, int angle) {
        ByteBuffer buffer = ByteBuffer.allocate(5);
        buffer.order(ByteOrder.LITTLE_ENDIAN);
        buffer.put( (byte) Message.ORDER);
        buffer.putShort((short)x);
    ...
    */
    ...
    public void confirmRobotFinished(int address) {
     ...
     /**
     * Method that tells the robot to reverse
     *
     * @param address The robots address
     */
    public void reverseRobot(int address) {
       protocolSettings.get(Message.ORDER).send(address, new byte[]{Message.REVERSE_UPDATE});
    }
